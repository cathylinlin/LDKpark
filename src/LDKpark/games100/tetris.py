...
class TetrisApp:
    ...
    def setup_ui(self):
        ...
        # 分数
        tk.Label(
            info_frame, text="分数:", fg="white", bg="#2b2b2b", font=("Arial", 12)
        ).pack(anchor="w")
        self.score_label = tk.Label(
            info_frame, text="0", fg="yellow", bg="#2b2b2b", font=("Arial", 16, "bold")
        )
        self.score_label.pack(anchor="w")

        # 等级
        tk.Label(
            info_frame, text="等级:", fg="white", bg="#2b2b2b", font=("Arial", 12)
        ).pack(anchor="w", pady=(10, 0))
        self.level_label = tk.Label(
            info_frame, text="1", fg="cyan", bg="#2b2b2b", font=("Arial", 16, "bold")
        )
        self.level_label.pack(anchor="w")

        # 下一个方块预览
        tk.Label(
            info_frame, text="下一个:", fg="white", bg="#2b2b2b", font=("Arial", 12)
        ...
        tk.Button(btn_frame, text="开始游戏", command=self.start_game, width=12).pack(
            pady=2
        )
        tk.Button(
            btn_frame, text="设置键位", command=self.open_settings, width=12
        ).pack(pady=2)
        tk.Button(btn_frame, text="加载BGM", command=self.load_bgm, width=12).pack(
            pady=2
        )
    ...
    def run_game_loop(self):
        ...
        self.game_loop_after_id = self.root.after(speed, self.run_game_loop)
    ...
    def draw_next_shape(self):
        ...
        for r, c in shape:
            ...
            self.next_canvas.create_rectangle(
                draw_x, draw_y, draw_x + 20, draw_y + 20, fill=color, outline="white"
            )
        ...
        def play_loop():
            if HAS_PYGAME:
                try:
                    ...
                    while (
                        not self.stop_bgm_event.is_set()
    ...
    def open_settings(self):
        ...
        def rebind(key_name):
            ...
            grabber.geometry(
                f"+{settings_win.winfo_rootx()}+{settings_win.winfo_rooty()}"
            )
        ...
        for name, key in self.keys.items():
            ...
            btn = tk.Button(
                frame, text=f"{name}: {key}", width=20, command=lambda n=name: rebind(n)
            )
